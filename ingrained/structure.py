...
class PartialCharge(object):
    ...
    with a calculator to simulate a STM image and 
    perform postprocessing manipulations.
    """
    ...
    def _get_stm_vol(self, z_below="", z_above=""):
        ...
        if self._parameter_check_z(z_below=z_below, z_above=z_above):
            ...
            # Get indices of volumetric slices to consider above and 
            # below the top surface (zmax)
            ...
            rhos  = np.swapaxes(np.swapaxes(
                    self.chgdata.copy(),0,2),1,2)[nzmin:nzmax,:,:]/\
    ...
    def _shift_sites(self,write_struct=False):
        ...
        for i in range(self.parchg.dim[0]):
            ...
            for j in  range(self.parchg.dim[1]):
                ...
                if self.zmax>.5:
                     new_dat[i].append(np.concatenate(
                             (dat_slice[zmax_index-int(ngzf/2):zmax_index], 
                     ...
                     new_dat[i].append(np.concatenate(
                                     (dat_slice[zmax_index+int(ngzf/2):],
                                     dat_slice[:zmax_index],
                                     dat_slice[zmax_index:zmax_index+
                                                                int(ngzf/2)])))
    ...
    def _get_stm_cell(self, z_below="", z_above="", r_val="", r_tol=""):
        ...
        in the imaging plane is NOT accounted for in the creation of the 
        image_cell. Use get_stm_image (wrapper) to return an image that 
        contains the proper skew!
        ...
        if self._parameter_check_r(r_val=r_val, r_tol=r_tol, rhos=rhos):
            ...
            # If absdiff between charge density value and r_val not is not 
            # within r_tol, switch pixels to off (i.e. -9E9)
            rhos[abs(rhos - r_val) >= r_tol] = -9E9

            # Find all the pixel values for which entire 
            # charge density volume is swiched off
            mask = np.sum(rhos,axis=0) == np.shape(rhos)[0]*-9E9

            # Find all on pixels and flip bottom values to top
            temp = (rhos.copy() > 0)[::-1,...]

            # Create image by finding depth of first nonzero rhos pixel 
            # (argmax) and subtract from max-depth
            image = ((nzmax-1)-np.argmax(temp,axis=0)).astype(np.float64)

            # Fill in all 'off' pixels with min 'on' pixels value (minus 5) 
    ...
    def simulate_image(self, sim_params=[]):
        ...
        Args:
            sim_params: (list) specifying the following simulation parameters
              ...
              - r_val: (float) isosurface charge density plane
              - r_tol: (float) tolerance to consider while determining 
                                                                  isosurface
              - x_shear: (float) fractional amt shear in x (+ to the right)
              - y_shear: (float) fractional amt shear in y (+ up direction)
              - x_stretch: (float) fractional amt stretch (+) or compression 
                                                                      (-) in x
              - y_stretch: (float) fractional amt stretch (+) or compression 
                                                                      (-) in y
              - rotation: (float) image rotation angle (in degrees) (+ is CCW)
              - pix_size: (float) real-space pixel size (Å)
              - sigma: (float) standard deviation for gaussian kernel used in 
                                                                postprocessing
        ...
        self._get_stm_cell(z_below=sim_params[0], z_above=sim_params[1], 
                           r_val=sim_params[2], r_tol=sim_params[3])

        # Construct a larger cell by repeating 
        # image_cell in both directions, NREPS = 8
    ...
    def simulate_image_random(self, rotation="", pix_size="", 
        ...
        Returns:
            A numpy array (np.float64) of the full simulated image.
        """
        sim_params = self._random_initialize(rotation=rotation, 
                                             pix_size=pix_size, 
    ...
    def _random_initialize(self, rotation="", pix_size="", 
                                       crop_height="", crop_width=""):
        """
        ...
        Returns:
            A numpy array of sim_params (tuple) specifying the 
            necessary simulation parameters for 'simulate_image'
        """
        ...
        if rotation != "":
            rotation = (rotation + random.choice(list(range(-360,360,90))) + 
                                                     random.uniform(-2,2))%360
        else:
        ...
        return [z_below, z_above, r_val, r_tol, x_shear, y_shear,
                x_stretch, y_stretch, rotation, pix_size, sigma, 
                crop_height, crop_width]
    ...
    def _image_size_check(self, img):
        ...
        if nrow > 35 and ncol > 35:
            ...
            raise Exception("ImageSizeError: {0} does not meet the minimum \
                            (37, 37) size requirement".format(np.shape(img)))
        return True

    def _parameter_check_z(self, z_below="", z_above=""):
        """
        Validate choice of depth parameters to ensure feasibility of STM 
                                                            image calculation
        ...
        if -1 <= z_below <= min([0.5*slab_thickness,4]):
            ...
            raise Exception("ParameterError: z_below = {0} outside valid range\
                            [{1} to {2:.5}]".format(z_below,-1,min(
                                                      [0.5*slab_thickness,4])))
                
        # Check to make sure z_above is within bounds of structure 
        # (within close proximity to surface, specifically)
        if 0 < z_above <= ((1 - self.zmax) * 0.5 * self.structure.lattice.c):
            pass
        else:
            raise Exception("ParameterError: z_above = {0} outside structure \
                            (must be <= {1:.5})".format(z_above,
                            ((1 - self.zmax)*0.5 * self.structure.lattice.c)))
        return True
    
    def _parameter_check_r(self, r_val="", r_tol="", rhos=""):
        """
        Validate choice of charge density parameters to ensure 
        feasibility of STM image calculation
        ...
        if rhos.max()/3 < r_val <  rhos.max():
            ...
            raise Exception("ParameterError: r_val = {0} outside valid range \
                            ({1:.5} to {2:.5})".format(r_val,rhos.max()/3,
                                                                  rhos.max()))

        # Check to make sure r_tol is within proper 
        # tolerance (0.0001 to 0.999*r_val)
        if 0.0001 < r_tol <  0.999*r_val:
            pass
        else:
            raise Exception("ParameterError: r_tol = {0} outside valid range \
                          ({1:.5} to {2:.5})".format(r_tol,0.0001,0.999*r_val))
        return True
    ...
    def _apply_lattice_shear(self, img):
        ...
        NOTE: This function has limited use cases. Add more for robust 
              handling! Currently assumes "a" lattice vector in x-direction and
              "b" lattice vector resolves into x and y components. 
        """
        # Get lattice vector a and b
        lva, lvb = self.parchg.structure.lattice.matrix[0], \
                                        self.parchg.structure.lattice.matrix[1]
        
        # Use count of nonzero entries as method to detect vectors
        # resolved along multiple directions
        if np.count_nonzero(lvb) == 1 and np.count_nonzero(lva) == 1:
            x_sh = 0
            y_sh = 0
        elif np.count_nonzero(lvb) == 2 and np.count_nonzero(lva) == 1:
            # Shear b vector in the x direction
            x_sh = -lvb[0]/np.linalg.norm(lva) 
            y_sh = 0
        else:
            raise Exception('Shear detected, but not accounted for in image \
                                                formed from volumetric grid!')
    ...
    def simulation_summary(self, iter="", verbose=False):
        ...
        z_below, z_above, r_val, r_tol, x_shear, y_shear, x_stretch, \
                 y_stretch,rotation, pix_size, sigma, crop_height, \
                                                 crop_width = self.sim_params
        summary = """Iteration {0}:
        • (z_below, z_above) (Å)    :  {1}
        • (r_val, r_tol)            :  {2}
        • (x, y) shear (frac)       :  {3}
        ...
        • rotation (deg CCW)        :  {6}
        • pix_size (Å)              :  {7}
        • img_size (pixels)         :  {8}""".format(iter,(z_below, z_above),
                                                        (r_val, r_tol),
                                                        (x_shear, y_shear),
                                                        (x_stretch, y_stretch),
                                                         sigma,rotation,
                                                         pix_size,(crop_height,
                                                                   crop_width))
        if verbose:
            print(summary)
        else:
...
class Bicrystal(object):
    """
    This class contains two oriented supercell slabs prepared from structure 
    queries to the Materials Project, as well as a calculator to fuse the slabs
    ...
    def __init__(self, config_file="", write_poscar=False, poscar_file=""):
        ...
        if poscar_file != "":
            ...
            try:
                shutil.copytree(os.path.dirname(__file__)+'/simulation', 
                                os.getcwd()+'/simulation')
            except:
                pass
    ...
    def _get_base_structure(self, top_grain_structure, 
                                        bot_grain_structure, constraints):
        """
        Take grains and fuse them into a simple bicrystal, 
        ...
        Args:
            ...
            constraints (dict): dictionary of geometric constraints 
                                                    for bicrystal construction
        
        Returns:
            The basic fused bicrystal (pymatgen structure) w/ 
                                    corresponding ingrained.contruct.TopGrain() 
            and ingrained.contruct.BottomGrain() objects after 
                                        average expand/contract values applied
        """
        structure, top_grain, bot_grain, strain_info= \
                        self._fuse_grains(top_grain_structure, 
                                          bot_grain_structure, constraints)
        structure = self._adjust_interface_width(structure=structure, 
                            interface_width_1=constraints['interface_1_width'],
                            interface_width_2=constraints['interface_2_width'])
        structure = self._remove_interface_collisions(structure=structure, 
                       collision_removal_1=constraints['collision_removal'][0],
                       collision_removal_2=constraints['collision_removal'][1])
    
        return structure, top_grain, bot_grain, strain_info
    
    def _fuse_grains(self, top_grain_structure, 
                                 bot_grain_structure, constraints):
        """
        Take current grain structures and expand/contract in width/depth to 
        minimize strain between grains and assign same width/depth computed as 
        the average of the ideal expand/contract values for each grain. 

        Args:
            top_grain_structure (): an ingrained.contruct.TopGrain() object
            bot_grain_structure (): an ingrained.contruct.BottomGrain() object
            constraints (dict): dictionary of geometric constraints for 
                                                        bicrystal construction
            
        Return:
            A bicrystal (pymatgen structure) from fusion of 
                                                    top and bottom grains
        """
        # Find integer expansions that minimize strain between lattices
        top_n_width, bot_n_width, tol_width = \
                self._find_optimal_expansion(top_grain_structure.width, 
                                             bot_grain_structure.width,
                                             min_len=constraints['min_width'], 
                                             max_len=constraints['max_width'])
        top_n_depth, bot_n_depth, tol_depth = \
                self._find_optimal_expansion(top_grain_structure.depth, 
                                             bot_grain_structure.depth,
                                             min_len=constraints['min_depth'], 
                                             max_len=constraints['max_depth'])

        # Apply appropriate integer expansions to grain structures
        top_grain_structure.structure.make_supercell(
                                                   [top_n_width,1,top_n_depth])
        bot_grain_structure.structure.make_supercell(
                                                   [bot_n_width,1,bot_n_depth])
        
        # Get expanded width/depth values of the grains 
        widths = [top_grain_structure.structure.lattice.a,
                                      bot_grain_structure.structure.lattice.a]
        depths = [top_grain_structure.structure.lattice.c,
                                      bot_grain_structure.structure.lattice.c]
        ...
        # Insert top_grain info into gb structure 
        # (on top of the exisiting 'bottom' structure)    
        ...
        gb_full.set_positions(np.vstack([np.vstack([gb_pos[:,2],
                                                    gb_pos[:,0]]),
                                                    gb_pos[:,1]]).T) 
        ...
        return gb_full_structure, top_grain_structure, bot_grain_structure, \
                            [strain_top_width,strain_top_depth,
                             strain_bot_width,strain_bot_depth]
    
    def _adjust_interface_width(self, structure="", 
                                    interface_width_1=0, interface_width_2=0):
        """
        Adjust spacing between both interfaces between grains, 
        with option to remove collision conflicts

        Args:
            structure (pymatgen structure): bicrystal structure
            interface_width_1: (float) spacing b/w max pos of bottom grain and
                                                       min pos of top grain (Å)
            interface_width_2: (float) spacing b/w max pos of top grain and
                                                    min pos of bottom grain (Å)

        Return:
            A bicrystal (pymatgen structure) that reflects 
                                                    specified interface widths
        """   
        # Get copy of current structure  
        bc = structure.copy()   
        if interface_width_1 != 0:
            # Update atom coordinate positions based on modified length in 
            # the c-direction from interface_width_1
            new_crds, new_spec = [], []
            for idx in range(len(bc)):
                cx,cy,cz = bc.cart_coords[idx]
                if bc.cart_coords[idx][2] >= bc.lattice.c/2:
                    new_crds.append([cx/bc.lattice.a,cy/bc.lattice.b,
                                     (cz+interface_width_1)/\
                                     (bc.lattice.c+interface_width_1)])
                else:
                    new_crds.append([cx/bc.lattice.a,cy/bc.lattice.b,cz/\
                                             (bc.lattice.c+interface_width_1)])
                new_spec.append(str(bc.species[idx]).split('Element')[0])
            
            # Create new 'Lattice' from parameters and a new 
            # pymatgen 'Structure' with updated positions
            lattice = Lattice.from_parameters(a=bc.lattice.a, b=bc.lattice.b, 
                                            c=(bc.lattice.c+interface_width_1),
        ...
        if interface_width_2 != 0:
            ...
            # Update atom coordinate positions based on modified length in 
            # the c-direction from interface_width_2
            new_crds, new_spec = [], []
            for idx in range(len(bc)):
                cx,cy,cz = crds[idx]
                new_crds.append([cx/bc.lattice.a,cy/bc.lattice.b,cz/\
                                 (np.max(crds[:,2]) + interface_width_2/2)])
                new_spec.append(str(bc.species[idx]).split('Element')[0])
            
            # Create new 'Lattice' from parameters and a new pymatgen 
            # 'Structure' with updated positions
            lattice = Lattice.from_parameters(a=bc.lattice.a, b=bc.lattice.b, 
                                              c=(np.max(crds[:,2]) + \
                                                          interface_width_2/2),
                                              alpha=90, beta=90, gamma=90)
    ...
    def _remove_interface_collisions(self, structure="", 
                                     collision_removal_1=False, 
                                     collision_removal_2=False, 
                                     collision_distance=1):
        """
        Remove atoms at the interface that are within 'collision_distance' of 
        another atom, starting removal with atoms at the bottom of the 
        interface region
        
        Args:
            structure (pymatgen structure): bicrystal structure
            collision_removal_1 (boolean): remove atoms within 
                              'collision_distance' in volume around interface_1
            collision_removal_2 (boolean): remove atoms within 
                              'collision_distance' in volume around interface_2
            collision_distance (float): atoms less than or equal to 
                                    this distance are considered collisions
        
        Return:
            A bicrystal (pymatgen structure) with collisions removed 
            within the interface volumes
        """
        # Get copy of current structure
        bc = structure.copy()
        ...
        if collision_removal_1:
            ...
            # Define region surrounding interface where we check 
            # for conflict (3Å region)
            iw_zone = ((bc.lattice.c/2)-1.5,(bc.lattice.c/2)+1.5)
            
            # Retrieve all coordinates belonging to the iw_zone
            interface_coords = pos[np.where((pos[:,2]>=iw_zone[0]) & \
                                            (pos[:,2]<=iw_zone[1]))]
            
            # Ensure they are sorted such that lower 
            # coordinates are tested first
            interface_coords =interface_coords[interface_coords[:,2].argsort()]
            
            for coord in interface_coords:
                # Get array of distances between coord and 
                # coords of current structure
        ...
        if collision_removal_2:
            ...
            # Define region surrounding interface where we 
            # check for conflict (3Å region)
            iw_zone = (0,1.5)
    ...
    def _get_image_cell(self, defocus=1, interface_width=0, 
                                pix_size=0.15, view=False):       
        """
        Calculate a single HAADF STEM image cell from bicrystal structure. 
    
        Args:
            defocus: (float) controls degree to which edges blur in 
                                                           microscopy image (Å)
            interface_width: (float) spacing b/w max pos of 
                                      bottom_grain and min pos of top grain (Å)
            pix_size: (float) real-space pixel size (Å)
        ...
        # Apply additional interface width adjustments 
        # (on top of interface_1_width which is set during initialization)
        bc = self._adjust_interface_width(structure=bc, 
                                          interface_width_1=interface_width)
        bc = self._remove_interface_collisions(structure=bc, 
                                               collision_removal_1=True)
        ...
        with open(os.getcwd()+'/simulation/SAMPLE.XYZ', "w") as sf:
            ...
            save_coords = np.vstack([np.array(bc.atomic_numbers),
                                     coords_list[:,1],
                                     bc.lattice.c-coords_list[:,2],
                                     coords_list[:,0],
                                     np.ones(len(coords_list[:,0])),
                                     0.076*np.ones(len(coords_list[:,0]))]).T
    ...
    def simulate_image(self, sim_params=[]):
        ...
        Args:
            sim_params: (list) specifying the following simulation parameters
              - pix_size: (float) real-space pixel size (Å)
              - interface_width: (float) spacing b/w max pos of bottom_grain 
                                                  and min pos of top grain (Å)
              - defocus: (float) controls degree to which edges blur 
                                                       in microscopy image (Å)
              - x_shear: (float) fractional amt shear in x (+ to the right)
              - y_shear: (float) fractional amt shear in y (+ up direction)
              - x_stretch: (float) fractional amt stretch (+) or 
                                                  compression (-) in x
              - y_stretch: (float) fractional amt stretch (+) or 
                                                  compression (-) in y
              - crop_height: (int) final (cropped) image height in pixels
              - crop_width: (int) final (cropped) image width in pixels
        ...
        if self.pix_size != None:
            sim_params[0] = sorted((0.945*self.pix_size, sim_params[0], 
                                                      1.055*self.pix_size))[1]
        else:
        ...
        img_cell, new_struct = self._get_image_cell(defocus=sim_params[2], 
                                                interface_width=sim_params[1], 
        ...
        # Resize image based on pixel_size
        img_tiled = iop.apply_resize(img_tiled, 
                                     np.array([int(a) for a in 
                                               np.shape(img_tiled) * \
                                               np.array(self.lw[::-1]) * \
                                               (1./sim_params[0])]))
        ...
        sim_params[-1] = sorted((35,int(2 * np.floor(sim_params[-1]/2) + 1),
                                  int(2 * np.floor((np.shape(img_tiled)[1]-2)/\
                                                                   2) + 1)))[1]
        sim_params[-2] = sorted((35,int(2 * np.floor(sim_params[-2]/2) + 1),
                                  int(2 * np.floor((np.shape(img_tiled)[0]-2)/\
                                                                   2) + 1)))[1]
    ...
    def simulate_image_random(self, pix_size="", crop_height="", crop_width=""): 
        ...
        Returns:
            A numpy array (np.float64) of the full simulated image.
        """
        sim_params = self._random_initialize(pix_size=pix_size, 
                                             crop_height=crop_height, 
                                             crop_width=crop_width)
        img_rand, __ = self.simulate_image(sim_params)
        return img_rand
    ...
    def _random_initialize(self, pix_size="", crop_height="", crop_width=""):
        ...
        return [pix_size, interface_width, defocus, x_shear, y_shear, 
                                 x_stretch, y_stretch, crop_height, crop_width]
    ...
    def _find_optimal_expansion(x, y, min_len=5, max_len=100):
        ...
        dmat = distance.cdist(np.array(alist).reshape(-1,1),
                              np.array(blist).reshape(-1,1),'euclidean')
        result = min((min((v, c) for c, v in enumerate(row)), r) for 
                                                     r, row in enumerate(dmat))
        return int(alist[result[1]]/x),int(blist[result[0][1]]/y), np.min(dmat)
    
    @staticmethod
    def _enforce_pb(pymatgen_structure):
        """
        Wrap atoms that have fixed coords outside cell back to their coord in 
        cell. Notice this sometimes happens when using 
        AseAtomsAdaptor.get_structure() to convert ASE to pymatgen
        ...
        for i in range(len(s.frac_coords)):
            s[i] = s[i].specie, [s[i].frac_coords[0]%1, 
                                 s[i].frac_coords[1]%1, 
                                 s[i].frac_coords[2]%1]
        return s
    ...
    def _resolve_pb_conflicts(pymatgen_structure):
        """
        If atoms near the 0 bound (width or depth only) interfere w/ 
                                      atoms @ max bounds w/ PBC's, delete them!
    ...
    def simulation_summary(self, iter="", verbose=False):
        ...
        """
        # Unpack parameters specific to simulation
        pix_size, interface_width, defocus, x_shear, y_shear, x_stretch, \
                           y_stretch, crop_height, crop_width = self.sim_params
        summary = """Iteration {0}:
        • pix_size (Å)              :  {1}
        • interface width (Å)       :  {2}
        • defocus (Å)               :  {3}
        • (x, y) shear (frac)       :  {4}
        • (x, y) stretch (frac)     :  {5}
        • img_size (pixels)         :  {6}""".format(iter,pix_size, 
                                                    interface_width, 
                                                    defocus,
                                                    (x_shear, y_shear),
                                                    (x_stretch, y_stretch), 
