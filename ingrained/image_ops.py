...
def apply_rotation(img, rotation):
    ...
    edge_length = int(np.min(np.shape(img))/(np.cos(np.radians(np.abs(rotation)%90)) + np.sin(np.radians(np.abs(rotation)%90))))-5
...
def apply_shear(img, x_sh, y_sh):
    ...
    afine_tf = transform.AffineTransform(matrix=np.array([[1,x_sh,-(ncols*x_sh)/2],[y_sh,1,-(nrows*y_sh)/2],[0,0,1]]))
    
    # Apply transform to image data
    img_shear = transform.warp(img, inverse_map=afine_tf)
    
    # Find edge_length for a fully inscribed square (this is a conservative approach)
    edge_length = int(np.min([(np.shape(img)[1] - np.shape(img)[1]*np.abs(x_sh)),(np.shape(img)[0] - np.shape(img)[0]*np.abs(y_sh))]))
...
def apply_stretch(img, x_st, y_st):
    ...
    img_stretch = transform.resize(img, (int((1+y_st) * nrows),int((1+x_st) * ncols)), preserve_range=True, anti_aliasing=True)
...
def apply_quantize_downsample(img, factor=2):
    """
    Quantize pixel values (16 levels of greyscale) and downsample image by factor (reduce spatial resolution) 
    ...
    new_size = np.array([int(2 * np.floor(a/2) + 1) for a in (np.array(np.shape(img))/factor)])
...
def score_ssim(img1,img2,win_size=35):
    ...
    Args:
        ...
        win_size: (int) length of sliding window used in comparison (must be odd value)
...
def read_SXM_file(sxm_path):
    ...
    if sxm_path.split('.')[-1] == 'txt':
        with open(sxm_path,'r') as f:
            ...
            image_details = [a.strip("\n").split("#")[-1].strip() for a in contents[0:4]]
            image_lines = [[float(b) for b in a.split("\t") if b != ""] for a in contents[4:-1]]
            image = np.array(image_lines)
            d = {a.split(":")[0].strip():a.split(":")[1].strip() for a in image_details}
        ...
        ps_width  = spm_obj.size['real']['y'] * to_angstrom[spm_unit]/np.shape(image)[1]
        ps_height = spm_obj.size['real']['x'] * to_angstrom[spm_unit]/np.shape(image)[0]
    else:
        print("Unrecognized file format {0}".format(sxm_path.split('.')[-1]))
    try: 
        assert ps_width == ps_height
        # Get min pixel value of image and used to fill in any pixels that are corrupt
...
def read_dm3_file(dm3_path):
    ...
    d["Experiment Pixel Size"] = dm3lib.DM3(dm3_path).pxsize[0]*to_angstrom[exp_unit]
    return d
